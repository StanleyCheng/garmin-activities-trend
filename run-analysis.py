...
def load_and_prepare_data(file_path):
    ...
    # New exports use the normalized pipeline schema; keep legacy workbooks usable.
    if "date" in df.columns:
        ...
        df["Pace (min/km)"] = pd.to_numeric(df["Pace (min/km)"], errors="coerce") / 60
    else:
        df["Pace (min/km)"] = df["Pace (min/km)"].apply(convert_pace_to_minutes)
    ...
    running_df = df[
        (df["activityType"].astype("string").str.contains("run", case=False, na=False))
        & (df["Distance (km)"] <= 700)
    ].copy()
...
def plot_running_data(
    running_df, start_date=None, end_date=None, chart_type="bar", output_file=None
):
    ...
    if chart_type.lower() == "bar":
        ...
        for x, y in zip(
            range(len(monthly_stats)), monthly_stats["Total Distance (km)"]
        ):
            plt.text(x, y + 5, f"{y:.1f}", color="black", ha="center", va="bottom")
...
def main():
    ...
    try:
        ...
        print(f"Error: {e}")
        print("Make sure the input file was produced by get-garmin.py.")
        return 1
